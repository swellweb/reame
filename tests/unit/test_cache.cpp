...
using reame::cache::CacheError;
using reame::cache::CacheManager;
using reame::cache::DiskCacheStore;
using reame::cache::KVCacheSerializer;
...
struct TempDir {
    ...
    TempDir() {
        path = fs::temp_directory_path() /
               ("reame-cache-test-" + std::to_string(counter++));
        fs::remove_all(path);
...
TEST_CASE("compress/decompress round-trips arbitrary bytes") {
    const auto data = bytes("reame kv cache payload 123");
