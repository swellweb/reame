...
TOOLS="${REAME_LLAMA_TOOLS:-}"
if [[ -z "$TOOLS" ]]; then
    echo "Building llama.cpp tools (one-off)..."
    cmake -S "$(dirname "$0")/../third_party/llama.cpp" -B /tmp/reame-llama-tools \
    ...
    cmake --build /tmp/reame-llama-tools --parallel \
        --target llama-imatrix llama-quantize > /dev/null
    TOOLS=/tmp/reame-llama-tools/bin
fi
